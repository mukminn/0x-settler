# 0x Settler Documentation

Welcome to the 0x Settler documentation! This directory contains comprehensive documentation for developers, contributors, and users of the 0x Settler project.

## Documentation Index

### For Developers

- **[TESTING.md](TESTING.md)** - Comprehensive testing guide
  - Test structure and organization
  - How to write tests
  - Error handling tests
  - Best practices
  - Gas testing

- **[CONTRIBUTING.md](CONTRIBUTING.md)** - Contribution guidelines
  - Getting started
  - Development setup
  - Code style
  - Pull request process
  - Good first issues

### Main Documentation

- **[../README.md](../README.md)** - Project overview and main documentation
- **[../CLAUDE.md](../CLAUDE.md)** - Development guidelines and patterns
- **[../GOOD_FIRST_ISSUES.md](../GOOD_FIRST_ISSUES.md)** - Guide for new contributors

## Quick Links

### Getting Started

1. Read the [main README](../README.md) for project overview
2. Check [CONTRIBUTING.md](CONTRIBUTING.md) for contribution guidelines
3. Review [TESTING.md](TESTING.md) for testing practices

### For Contributors

- [Good First Issues Guide](../GOOD_FIRST_ISSUES.md)
- [Development Guidelines](../CLAUDE.md)
- [Testing Documentation](TESTING.md)

### For Users

- [Main README](../README.md) - Integration guide
- [0x API Documentation](https://0x.org/docs/api)
- [0x Developer Resources](https://0x.org/docs/developer-resources)

## Documentation Structure

```
docs/
├── README.md           # This file
├── TESTING.md          # Testing documentation
└── CONTRIBUTING.md     # Contribution guidelines

../
├── README.md           # Main project documentation
├── CLAUDE.md           # Development guidelines
└── GOOD_FIRST_ISSUES.md # Guide for new contributors
```

## Contributing to Documentation

Documentation improvements are always welcome! See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines.

Common documentation tasks:
- Fix typos and grammar
- Improve clarity
- Add examples
- Update outdated information
- Add missing documentation

## Resources

- [0x Protocol Documentation](https://0x.org/docs)
- [Foundry Documentation](https://book.getfoundry.sh/)
- [Solidity Documentation](https://docs.soliditylang.org/)
- [Permit2 Documentation](https://github.com/Uniswap/permit2)

